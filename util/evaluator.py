...
def evaluate(task, golddir, outfile):
    ...
    X_train, y_train = feature_extractor(gold)

    train_algo(X_train=X_train, y_train=y_train)
...
def feature_extractor(gold):
    X_train = []
    y_train = []
    for ent in gold['CLASS']:
        word = ent.split("|")[2]
        label = ent.split("|")[3]

        X_train.append([{'word': word}])
...
def train_algo(X_train, y_train):
    ...
    output = pd.DataFrame([y_train, y_pred])
